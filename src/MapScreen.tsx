...
import { 
  ...
  Bot, HeartHandshake, Paperclip, MessageSquarePlus, History, User, Lightbulb, Info
} from 'lucide-react';
...
interface SafetyTip {
  ...
  lat?: number;
  lng?: number;
  category: 'general' | 'wheelchair' | 'night' | 'lgbtq' | 'other';
  createdAt: any;
}
...
function MapEvents({ onMoveEnd, onMapClick }: { onMoveEnd: (center: [number, number]) => void, onMapClick: (e: any) => void }) {
  const map = useMapEvents({
    ...
    click: (e) => {
      // Pass the event to handle coordinates
      onMapClick(e);
    }
  });
  return null;
...
const createPlaceIcon = (category: string, isLGBTQ: boolean, isAccessible: boolean) => {
  const emoji = category === 'cafe' ? '☕' : category === 'clinic' ? '🏥' : category === 'shelter' ? '🏠' : category === 'store' ? '🛒' : '📍';
  const borderColor = isLGBTQ ? '#FF2D55' : isAccessible ? '#007AFF' : '#34C759';
  
  return L.divIcon({
    className: 'bg-transparent border-none',
    html: `
      <div style="position: relative; display: flex; align-items: center; justify-content: center; width: 32px; height: 32px;">
        <div style="width: 32px; height: 32px; border-radius: 50%; background-color: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border: 2px solid ${borderColor}; display: flex; align-items: center; justify-content: center; font-size: 16px;">
          ${emoji}
        </div>
        ${isAccessible ? `<div style="position: absolute; bottom: -4px; right: -4px; width: 16px; height: 16px; background: white; border-radius: 50%; border: 1px solid #bfdbfe; display: flex; align-items: center; justify-content: center; font-size: 10px;">♿</div>` : ''}
      </div>
    `,
    iconSize: [32, 32],
    iconAnchor: [16, 16]
...
const tipIcon = L.divIcon({
  className: 'bg-transparent border-none',
  html: `
    <div style="width: 32px; height: 32px; border-radius: 50%; background-color: #FFD60A; border: 2px solid white; box-shadow: 0 4px 10px rgba(0,0,0,0.2); display: flex; align-items: center; justify-content: center; transform: scale(1); transition: transform 0.2s ease-in-out;">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#8B4513" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A5 5 0 0 0 8 8c0 1.3.5 2.6 1.5 3.5.8.8 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg>
    </div>
  `,
  iconSize: [32, 32],
  iconAnchor: [16, 16]
...
export default function MapScreen() {
  ...
  const [roughRoads, setRoughRoads] = useState<RoughRoad[]>([]);
  ...
  const [newRoad, setNewRoad] = useState({ name: '', description: '', severity: 'medium' as 'low' | 'medium' | 'high' });
  const [selectedRoad, setSelectedRoad] = useState<RoughRoad | null>(null);
  const [safetyTips, setSafetyTips] = useState<SafetyTip[]>([]);
  const [showTipModal, setShowTipModal] = useState(false);
  const [newTip, setNewTip] = useState({ text: '', category: 'general' as const });
  const [selectedTip, setSelectedTip] = useState<SafetyTip | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'place' | 'danger_zone' | 'place_comment' | 'danger_zone_comment' | 'safety_tip', id: string, parentId?: string } | null>(null);
  ...
  const [newDanger, setNewDanger] = useState({ name: '', description: '', radius: 100, severity: 'medium' as const, type: 'danger' as 'danger' | 'rough_road', isTemporary: true, duration: 24 });
  ...
  const [dismissedDangerZoneId, setDismissedDangerZoneId] = useState<string | null>(null);
    ...
    const seedData = async () => {
      ...
      const defaultPlaces = [
        {
          ...
          description: 'Trung tâm thương mại có lối đi cho xe lăn, thang máy rộng rãi và nhà vệ sinh dành cho người khuyết tật.',
          category: 'store',
          lat: 10.3847,
          lng: 105.4367,
          addedBy: user.uid,
        ...
        },
        {
          ...
          description: 'Bệnh viện tuyến tỉnh với cơ sở hạ tầng tiếp cận tốt cho xe lăn, có đường dốc và thang máy.',
          category: 'clinic',
          lat: 10.3696,
          lng: 105.4360,
          addedBy: user.uid,
        ...
        {
          name: 'Đại học An Giang (Khu trung tâm)',
          description: 'Khuôn viên trường đại học rộng rãi, có đường dốc cho xe lăn tại các tòa nhà chính.',
          category: 'shelter',
          lat: 10.3698,
          lng: 105.4315,
          addedBy: user.uid,
        ...
        },
        {
          ...
          description: 'Siêu thị có lối đi bằng phẳng, thang cuốn trệt và thang máy, thuận tiện cho người đi xe lăn.',
          category: 'store',
          lat: 10.3820,
          lng: 105.4403,
          addedBy: user.uid,
      ...
      try {
        for (const place of defaultPlaces) {
          const q = query(collection(db, 'safe_places'), where('name', '==', place.name));
          ...
          if (snapshot.empty) {
            ...
            // Force update coordinates to ensure they are correct
            const docId = snapshot.docs[0].id;
            await updateDoc(doc(db, 'safe_places', docId), {
              lat: place.lat,
              lng: place.lng
            });
            ...
            if (snapshot.docs.length > 1) {
              for (let i = 1; i < snapshot.docs.length; i++) {
                await deleteDoc(doc(db, 'safe_places', snapshot.docs[i].id));
              }
            }
          }
        }
  ...
  const [userPrimaryIdentity, setUserPrimaryIdentity] = useState<string | null>(null);
    ...
    if (user) {
      getDoc(doc(db, 'users', user.uid)).then(docSnap => {
        if (docSnap.exists()) {
          ...
          if (docSnap.data().primaryIdentity) {
            setUserPrimaryIdentity(docSnap.data().primaryIdentity);
            if (docSnap.data().primaryIdentity === 'pwd') {
              setActiveFilters(prev => prev.includes('wheelchair') ? prev : [...prev, 'wheelchair']);
              setRoutingMode('walking'); // Default to walking/wheelchair mode for PWD
            } else if (docSnap.data().primaryIdentity === 'lgbt') {
              setActiveFilters(prev => prev.includes('lgbtq') ? prev : [...prev, 'lgbtq']);
            }
          }
        }
      });
  ...
  const handleAIRecommendPlaces = async () => {
    ...
    try {
      // Find top 10 closest places
      const placesWithDist = places.map(p => ({
        ...p,
        distance: L.latLng(userLocation).distanceTo(L.latLng(p.lat, p.lng))
      })).sort((a, b) => a.distance - b.distance).slice(0, 10);

      const placesJson = placesWithDist.map(p => ({
        name: p.name,
        category: p.category,
        ...
        genderNeutralRestroom: p.genderNeutralRestroom
      }));

      const userMsg = `Đề xuất địa điểm an toàn gần tôi.`;
      const promptText = `Tôi đang ở vị trí có tọa độ [${userLocation[0]}, ${userLocation[1]}]. 
Dưới đây là danh sách các địa điểm an toàn gần tôi (đã sắp xếp theo khoảng cách):
${JSON.stringify(placesJson, null, 2)}

Các tiêu chí ưu tiên của tôi (nếu có): ${activeFilters.length > 0 ? activeFilters.join(', ') : 'Không có'}.

Hãy đóng vai một trợ lý an toàn, phân tích và đề xuất 2-3 địa điểm phù hợp nhất cho tôi từ danh sách trên. Giải thích lý do tại sao (dựa trên khoảng cách và tiêu chí ưu tiên) và cung cấp khoảng cách ước tính. Trả lời ngắn gọn, thân thiện và dễ đọc.`;
      ...
      if (!chatId) {
        const newChatRef = doc(collection(db, 'users', user.uid, 'chats'));
        await setDoc(newChatRef, {
          title: "Đề xuất địa điểm an toàn",
          updatedAt: serverTimestamp()
        });
        chatId = newChatRef.id;
        setCurrentAIChatId(chatId);
  ...
  const buildRouteSegments = (coords: [number, number][], allZones: DangerZone[], allRoughRoads: RoughRoad[]) => {
    ...
    for (let i = 0; i < coords.length; i++) {
      ...
      const activeZones = allZones.filter(z => {
        ...
        if (z.type === 'rough_road' && userPrimaryIdentity !== 'pwd') return false;
        return L.latLng(coord).distanceTo(L.latLng(z.lat, z.lng)) <= z.radius;
      });

      if (userPrimaryIdentity === 'pwd') {
         allRoughRoads.forEach(road => {
            for (const rp of road.positions) {
               if (L.latLng(coord).distanceTo(L.latLng(rp[0], rp[1])) < 50) {
                  if (!activeZones.find(z => z.id === road.id)) {
                     activeZones.push({
                        id: road.id,
                        name: road.name,
                        description: road.description,
                        type: 'rough_road',
                        severity: road.severity || 'medium',
                        lat: rp[0],
                        lng: rp[1],
                        radius: 50,
                        addedBy: road.addedBy,
    ...
    try {
      ...
      const evaluateRoute = (coordinates: [number, number][]) => {
        ...
        coordinates.forEach((coord: [number, number], index: number) => {
          ...
          dangerZones.forEach(zone => {
            ...
            if (zone.type === 'rough_road' && userPrimaryIdentity !== 'pwd') return;

            const dist = L.latLng(coord).distanceTo(L.latLng(zone.lat, zone.lng));
            if (dist <= zone.radius) {
              if (!dangerousZonesOnRouteIds.has(zone.id)) {
                dangerousZonesOnRouteIds.add(zone.id);
                if (zone.type === 'rough_road') {
                  score += zone.severity === 'high' ? 1000 : zone.severity === 'medium' ? 100 : 10;
                } else {
                  score += zone.severity === 'high' ? 100 : zone.severity === 'medium' ? 50 : 10;
                }
              }
            }
          });
          ...
          if (userPrimaryIdentity === 'pwd') {
            roughRoads.forEach(road => {
              if (dangerousZonesOnRouteIds.has(road.id)) return;
              for (const rp of road.positions) {
                if (L.latLng(coord).distanceTo(L.latLng(rp[0], rp[1])) < 50) {
                  dangerousZonesOnRouteIds.add(road.id);
                  score += road.severity === 'high' ? 1000 : road.severity === 'medium' ? 100 : 10;
                  break;
                }
              }
            });
        ...
        return { ids: Array.from(dangerousZonesOnRouteIds), score };
      };
      ...
      if (data.routes.length > 0) {
        for (const route of data.routes) {
          ...
          const evaluation = evaluateRoute(coordinates);
          
          if (evaluation.score < minDangerScore || (evaluation.score === minDangerScore && route.distance < (bestDistance || Infinity))) {
            minDangerScore = evaluation.score;
            bestRouteCoords = coordinates;
            bestDangerZones = evaluation.ids;
            bestDistance = route.distance;
            bestDuration = route.duration;
          }
          if (minDangerScore === 0) break;
        }
      }

      // 2. If still hitting danger zones, try forcing a detour around the first danger zone
      if (minDangerScore > 0 && bestDangerZones.length > 0) {
        const firstDangerZone = dangerZones.find(z => z.id === bestDangerZones[0]);
        let detourCenter: [number, number] | null = null;
        let detourRadius = 200;

        if (firstDangerZone) {
           detourCenter = [firstDangerZone.lat, firstDangerZone.lng];
           detourRadius = firstDangerZone.radius + 200;
        } else {
           const firstRoughRoad = roughRoads.find(r => r.id === bestDangerZones[0]);
           if (firstRoughRoad && firstRoughRoad.positions.length > 0) {
             const midPoint = firstRoughRoad.positions[Math.floor(firstRoughRoad.positions.length / 2)];
             detourCenter = midPoint;
             detourRadius = 200;
           }
        }

        if (detourCenter) {
          const offsetLat = detourRadius / 111320;
          const offsetLng = detourRadius / (111320 * Math.cos(detourCenter[0] * Math.PI / 180));
          
          const detours = [
            [detourCenter[0] + offsetLat, detourCenter[1]], // North
            [detourCenter[0] - offsetLat, detourCenter[1]], // South
            [detourCenter[0], detourCenter[1] + offsetLng], // East
            [detourCenter[0], detourCenter[1] - offsetLng]  // West
          ];
          ...
          for (const detour of detours) {
            try {
              ...
              if (detourData.code === 'Ok' && detourData.routes.length > 0) {
                ...
                if (evaluation.score < minDangerScore || (evaluation.score === minDangerScore && detourRoute.distance < (bestDistance || Infinity))) {
                  minDangerScore = evaluation.score;
                  bestRouteCoords = coordinates;
                  bestDangerZones = evaluation.ids;
                ...
                if (minDangerScore === 0) break; // Found a perfectly safe route!
              }
            } catch (e) {
      ...
      if (bestRouteCoords.length > 0) {
        const segments = buildRouteSegments(bestRouteCoords, dangerZones, roughRoads);
        ...
        if (userPrimaryIdentity === 'pwd' && minDangerScore > 0) {
           const hasHighSeverity = bestDangerZones.some(id => {
              const road = roughRoads.find(r => r.id === id);
              return road && road.severity === 'high';
           });
           if (hasHighSeverity) {
              alert("CẢNH BÁO: Không thể tìm thấy tuyến đường hoàn toàn tránh các đoạn đường rất xấu cho xe lăn. Vui lòng cực kỳ cẩn thận hoặc tìm người hỗ trợ nếu bạn vẫn muốn đi tuyến đường này.");
           } else {
              alert("Lưu ý: Tuyến đường này vẫn đi qua một số đoạn đường hơi gồ ghề. Hãy chú ý an toàn.");
           }
        }
      } else {
  ...
  useEffect(() => {
    ...
    const unsubRoads = onSnapshot(collection(db, 'rough_roads'), (snapshot) => {
      const roads = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as RoughRoad));
      setRoughRoads(roads);
    }, (error) => {
      handleFirestoreError(error, OperationType.LIST, 'rough_roads');
    });

    const unsubTips = onSnapshot(collection(db, 'safety_tips'), (snapshot) => {
      setSafetyTips(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as SafetyTip)));
    }, (error) => {
      handleFirestoreError(error, OperationType.LIST, 'safety_tips');
    });
  ...
  const confirmDangerLocation = () => {
    ...
    setNewDanger({ name: '', description: '', radius: 100, severity: 'medium', type: 'danger', isTemporary: true, duration: 24 });
  ...
  const handleDeleteDanger = async (zoneId: string) => {
    setDeleteTarget({ type: 'danger_zone', id: zoneId });
  };

  const handleDeletePlace = async (placeId: string) => {
    setDeleteTarget({ type: 'place', id: placeId });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (deleteTarget.type === 'danger_zone') {
        await deleteDoc(doc(db, 'danger_zones', deleteTarget.id));
        if (selectedDangerZone?.id === deleteTarget.id) {
          setSelectedDangerZone(null);
        }
      } else if (deleteTarget.type === 'place') {
        await deleteDoc(doc(db, 'safe_places', deleteTarget.id));
        if (selectedPlace?.id === deleteTarget.id) {
          setSelectedPlace(null);
        }
      } else if (deleteTarget.type === 'safety_tip') {
        await deleteDoc(doc(db, 'safety_tips', deleteTarget.id));
        if (selectedTip?.id === deleteTarget.id) {
          setSelectedTip(null);
        }
      } else if (deleteTarget.type === 'place_comment' && deleteTarget.parentId) {
        await deleteDoc(doc(db, `safe_places/${deleteTarget.parentId}/comments`, deleteTarget.id));
      } else if (deleteTarget.type === 'danger_zone_comment' && deleteTarget.parentId) {
        await deleteDoc(doc(db, `danger_zones/${deleteTarget.parentId}/comments`, deleteTarget.id));
      }
    } catch (error) {
    ...
    <div className="relative h-full w-full flex flex-col">
      <div className="p-4 bg-white/80 backdrop-blur-xl border-b border-gray-200/80 z-10 flex flex-col gap-3">
        <div className="flex justify-between items-center">
          <h1 className="text-[17px] font-semibold text-black">SafeHaven Map</h1>
          <div className="flex gap-2">
            ...
            {!isAddingMode && !isAddingDangerMode && !isDrawingRoad && (
              <div className="flex gap-2">
                <button 
                  onClick={() => { setIsDrawingRoad(true); setRoadPoints([]); }}
                  className="bg-[#A0522D] text-white p-2 rounded-full shadow-sm hover:bg-[#A0522D]/90 transition-colors"
                  title="Vẽ đoạn đường gồ ghề"
                >
                ...
                <button 
                  ...
                  className="bg-[#FFD60A] text-[#8B4513] p-2 rounded-full shadow-sm hover:bg-[#FFD60A]/90 transition-colors"
                  title="Chia sẻ mẹo an toàn"
                >
                  <Lightbulb size={22} />
                </button>
                <button 
      ...
      <div className="flex-1 relative z-0">
        ...
        >
          ...
          <MapEvents 
            ...
            onMapClick={(e) => {
              if (e.originalEvent && (e.originalEvent as any)._stopped) return;
              if (isDrawingRoad) {
                setRoadPoints(prev => [...prev, [e.latlng.lat, e.latlng.lng]]);
                return;
              }
                ...
                <Popup>
                  <div className="font-sans w-48">
                    <p className="font-bold text-[#FF3B30] flex items-center gap-1 mb-1 text-[13px]">
                      <AlertTriangle size={14} /> {segment.zones.some(z => z.type === 'rough_road') ? 'Đoạn đường gồ ghề' : 'Đoạn đường nguy hiểm'}
                    </p>
          ...
          {roughRoads.map(road => {
            const color = road.severity === 'high' ? '#8B0000' : road.severity === 'low' ? '#CD853F' : '#A0522D';
            return (
              <Polyline
                key={road.id}
                positions={road.positions}
                color={color}
                weight={road.severity === 'high' ? 8 : 6}
                dashArray="10, 10"
                eventHandlers={{
                  click: (e) => {
          ...
          {isDrawingRoad && roadPoints.length > 0 && (
            <Polyline positions={roadPoints} color="#A0522D" weight={6} dashArray="10, 10" />
          )}
          ...
          {dangerZones.map(zone => {
            ...
            const isRoughRoad = zone.type === 'rough_road';
            const baseColor = isRoughRoad 
              ? '#A0522D' // Sienna/Brown for rough road
              : (zone.severity === 'high' ? '#FF3B30' : zone.severity === 'medium' ? '#FF9500' : '#FFCC00');
            ...
            return (
              <Circle 
                ...
                eventHandlers={{
                  click: (e) => {
                    // Stop propagation so the map click handler doesn't immediately close it
                    (e.originalEvent as any)._stopped = true;
          ...
          {filteredPlaces.map(place => {
            const isLGBTQ = place.lgbtqFriendly || false;
            const isAccessible = place.wheelchairAccessible || false;
            
            const customMarkerIcon = createPlaceIcon(place.category, isLGBTQ, isAccessible);
            ...
            <div className="bg-white w-full h-[80vh] rounded-t-[32px] shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-300">
              ...
              <div className="p-4 bg-white border-b border-gray-100 flex items-center justify-between rounded-t-[32px]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-[17px] font-bold text-black leading-tight">Trợ lý SafeHaven</h1>
              ...
              {/* AI Quick Actions */}
              <div className="px-4 pt-2 pb-1 bg-white flex gap-2 overflow-x-auto no-scrollbar border-t border-gray-100">
                <button 
                  onClick={handleAIRecommendPlaces}
                  disabled={isAILoading}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#007AFF]/10 text-[#007AFF] rounded-full text-[13px] font-semibold whitespace-nowrap hover:bg-[#007AFF]/20 transition-colors disabled:opacity-50"
                >
                  <MapPin size={14} /> Gợi ý địa điểm an toàn
                </button>
              </div>
              ...
              <div className="p-4 bg-white pb-safe">
                ...
                <form onSubmit={handleAISend} className="flex gap-2 items-end">
                  ...
                  <textarea
                    ...
                    placeholder="Hỏi trợ lý SafeHaven..."
        ...
        {isDrawingRoad && (
          <div className="absolute bottom-6 left-0 right-0 px-4 z-[400] flex flex-col gap-2">
            <div className="bg-white/90 backdrop-blur-xl text-black text-center py-2 px-4 rounded-full shadow-sm text-[13px] font-medium mx-auto border border-gray-200/50">
              Chạm vào bản đồ để vẽ các điểm trên tuyến đường gồ ghề
            </div>
            <div className="flex gap-2">
              <button 
                onClick={() => { setIsDrawingRoad(false); setRoadPoints([]); }}
              ...
              <button 
                onClick={() => setRoadPoints(prev => prev.slice(0, -1))}
                disabled={roadPoints.length === 0}
                className="flex-1 bg-white text-gray-700 py-3.5 rounded-[14px] font-semibold shadow-[0_2px_10px_rgba(0,0,0,0.08)] border border-gray-100 text-[17px] disabled:opacity-50"
              >
                Hoàn tác
              </button>
              <button 
                ...
                className="flex-[2] bg-[#A0522D] text-white py-3.5 rounded-[14px] font-semibold shadow-[0_2px_10px_rgba(0,0,0,0.08)] flex items-center justify-center gap-2 text-[17px] disabled:opacity-50"
              >
                <Check size={20} />
                Xác nhận
              </button>
        ...
        {selectedPlace && !routingTo && (
          <div className={`absolute left-1/2 -translate-x-1/2 w-full max-w-lg z-[1001] bg-white/98 backdrop-blur-2xl shadow-[0_-12px_40px_rgba(0,0,0,0.12)] border-t border-gray-200/50 transition-all duration-300 ease-out flex flex-col ${showComments ? 'bottom-0 h-[65vh] rounded-t-[32px]' : 'bottom-4 px-4'}`}>
            <div className={`bg-white rounded-[28px] flex flex-col h-full overflow-hidden ${!showComments ? 'shadow-xl border border-gray-100' : ''}`}>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {selectedPlace.addedBy === user?.uid && (
                        <button 
                          onClick={() => setDeleteTarget({ type: 'place', id: selectedPlace.id })}
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {comment.authorId === user?.uid && (
                              <button 
                                onClick={() => setDeleteTarget({ type: 'place_comment', id: comment.id, parentId: selectedPlace.id })}
            ...
            <div className={`bg-white rounded-[28px] flex flex-col h-full overflow-hidden ${!showComments ? 'shadow-xl border border-gray-100' : ''}`}>
              ...
              <div className={`flex flex-col ${showComments ? 'overflow-hidden flex-1' : ''}`}>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1.5">
                      <div className={`flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-lg border ${selectedDangerZone.type === 'rough_road' ? 'bg-[#A0522D]/8 text-[#A0522D] border-[#A0522D]/10' : 'bg-[#FF3B30]/8 text-[#FF3B30] border-[#FF3B30]/10'}`}>
                        <AlertTriangle size={10} /> {selectedDangerZone.type === 'rough_road' ? 'Đường gồ ghề' : 'Khu vực nguy hiểm'}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {selectedDangerZone.addedBy === user?.uid && (
                        <button 
                          onClick={() => setDeleteTarget({ type: 'danger_zone', id: selectedDangerZone.id })}
                          className="text-gray-400 hover:text-red-500 transition-colors p-1"
                          title="Xóa khu vực"
                        >
                          <span className="material-symbols-outlined text-[20px]">delete</span>
                              ...
                              <button 
                                onClick={async () => {
                                  if (window.confirm('Bạn có chắc chắn muốn xóa báo cáo này?')) {
                                    try {
                                      await deleteDoc(doc(db, `danger_zones/${selectedDangerZone.id}/comments`, comment.id));
                                    } catch (error) {
                                      handleFirestoreError(error, OperationType.DELETE, `danger_zones/${selectedDangerZone.id}/comments/${comment.id}`);
                                    }
                                  }
                                }}
                                className="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1"
      ...
      {/* Selected Safety Tip Overlay */}
        {selectedTip && (
          <div className="absolute left-1/2 -translate-x-1/2 w-full max-w-lg z-[1001] bottom-4 px-4">
            <div className="bg-white rounded-[28px] flex flex-col overflow-hidden shadow-xl border border-gray-100 p-4 animate-in slide-in-from-bottom-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className="w-10 h-10 bg-[#FFD60A]/20 text-[#8B4513] rounded-full flex items-center justify-center">
                    <Lightbulb size={20} />
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Mẹo an toàn cộng đồng</p>
                    <p className="text-[12px] font-bold text-gray-400 capitalize">{selectedTip.category}</p>
                  </div>
                </div>
                <button 
                  onClick={() => setSelectedTip(null)}
                  className="bg-gray-100 hover:bg-gray-200 text-gray-500 p-1.5 rounded-full transition-all"
                >
                  <X size={16} />
                </button>
              </div>
              
              <p className="text-[15px] text-gray-800 leading-relaxed font-medium mb-4 italic">"{selectedTip.text}"</p>
              
              <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                <div className="flex items-center gap-2">
                  <img 
                    src={selectedTip.authorPhoto || `https://ui-avatars.com/api/?name=${selectedTip.authorName}`} 
                    alt={selectedTip.authorName} 
                    className="w-6 h-6 rounded-full object-cover bg-gray-100"
                  />
                  <span className="text-[12px] font-semibold text-gray-600">{selectedTip.authorName}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-[10px] text-gray-400">
                    {selectedTip.createdAt?.toDate ? selectedTip.createdAt.toDate().toLocaleDateString('vi-VN') : 'Gần đây'}
                  </span>
                  {selectedTip.authorId === user?.uid && (
                    <button 
                      onClick={async () => {
                        if (window.confirm('Bạn có chắc chắn muốn xóa mẹo an toàn này?')) {
                          try {
                            await deleteDoc(doc(db, 'safety_tips', selectedTip.id));
                            setSelectedTip(null);
                          } catch (error) {
                            handleFirestoreError(error, OperationType.DELETE, `safety_tips/${selectedTip.id}`);
                          }
                        }
                      }}
                      className="text-gray-400 hover:text-red-500 transition-colors p-1"
                      title="Xóa mẹo an toàn"
                    >
          ...
          <div className="absolute bottom-4 left-4 right-4 z-[1001] bg-white/95 backdrop-blur-xl p-3 rounded-2xl shadow-[0_-8px_30px_rgba(0,0,0,0.15)] border border-gray-200/60 flex flex-col gap-2 transition-all animate-in slide-in-from-bottom-10">
            ...
            <div className="flex gap-2 bg-gray-100/80 p-1 rounded-xl mt-0.5">
              ...
              <button
                ...
                disabled={isNavigating}
              >
                {userPrimaryIdentity === 'pwd' ? 'Xe lăn / Đi bộ' : 'Đi bộ'}
              </button>
            </div>
        ...
        {isInsideDanger && activeDangerZone && activeDangerZone.id !== dismissedDangerZoneId && (
          <div className="absolute top-24 left-4 right-4 z-[1001] bg-[#FF3B30] p-4 rounded-2xl shadow-2xl border border-[#FF3B30]/20 flex flex-col gap-3 animate-bounce-subtle">
              ...
              <button 
                onClick={() => {
                  if (!userLocation || places.length === 0) {
                    alert("Không tìm thấy địa điểm an toàn gần đây.");
                    return;
                  }
                  let nearest = places[0];
                  let minDist = L.latLng(userLocation).distanceTo(L.latLng(nearest.lat, nearest.lng));
                  for (let i = 1; i < places.length; i++) {
                    const dist = L.latLng(userLocation).distanceTo(L.latLng(places[i].lat, places[i].lng));
                    if (dist < minDist) {
                      minDist = dist;
                      nearest = places[i];
                    }
                  ...
                  setIsNavigating(true);
                  setDismissedDangerZoneId(activeDangerZone.id);
                }}
                className="flex-1 bg-white text-[#FF3B30] font-bold py-2 rounded-xl text-sm hover:bg-red-50 transition-colors shadow-sm"
              >
                Chỉ đường thoát
              </button>
              <button 
                onClick={() => setDismissedDangerZoneId(activeDangerZone.id)}
                className="px-4 py-2 bg-red-800 text-white font-bold rounded-xl text-sm hover:bg-red-900 transition-colors shadow-sm"
              >
                Bỏ qua
              </button>
            </div>
      ...
      {/* Safety Tip Modal */}
      {showTipModal && (
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm z-[2000] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[24px] w-full max-w-md flex flex-col shadow-2xl">
            <div className="flex justify-between items-center p-4 border-b border-gray-100">
              <h2 className="text-[17px] font-semibold text-[#FFD60A] flex items-center gap-2">
                <Lightbulb size={20} className="fill-current" /> Chia sẻ mẹo an toàn
              </h2>
              <button onClick={() => setShowTipModal(false)} className="p-1.5 text-[#8E8E93] hover:text-black bg-gray-100 rounded-full">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={async (e) => {
              e.preventDefault();
              if (!user || !newTip.text.trim()) return;
              setLoading(true);
              try {
                await addDoc(collection(db, 'safety_tips'), {
                  ...
                  authorId: user.uid,
                  authorName: userNickname || user.displayName || 'Người dùng ẩn danh',
                  authorPhoto: userAvatar || user.photoURL || '',
                  lat: draftLocation ? draftLocation[0] : null,
                  lng: draftLocation ? draftLocation[1] : null,
                  createdAt: serverTimestamp()
                });
                ...
                handleFirestoreError(error, OperationType.CREATE, 'safety_tips');
              } finally {
                setLoading(false);
              }
            }} className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-semibold mb-1 text-gray-700">Chủ đề</label>
                <select 
                  className="w-full rounded-xl bg-gray-50 p-3 outline-none border border-gray-100 focus:border-[#FFD60A] transition-all"
                  value={newTip.category}
                  onChange={e => setNewTip({...newTip, category: e.target.value as any})}
                >
                  <option value="general">Chung</option>
                  <option value="wheelchair">Xe lăn / Tiếp cận</option>
                  <option value="night">Buổi tối / Ánh sáng</option>
                  <option value="lgbtq">LGBTQ+ Friendly</option>
                  <option value="other">Khác</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1 text-gray-700">Lời khuyên của bạn</label>
                <textarea 
                  required
                  rows={4}
                  placeholder="VD: Khu vực này buổi tối hơi vắng, nên đi cùng bạn bè..."
                  className="w-full rounded-xl bg-gray-50 p-3 outline-none border border-gray-100 focus:border-[#FFD60A] transition-all resize-none"
                  value={newTip.text}
                  onChange={e => setNewTip({...newTip, text: e.target.value})}
                />
              </div>
              <p className="text-[11px] text-gray-400 italic">
                {draftLocation ? '📍 Mẹo này sẽ được gắn tại vị trí bạn đã chọn trên bản đồ.' : '💡 Mẹo này sẽ là lời khuyên chung cho cộng đồng.'}
              </p>
              <button 
                type="submit"
                disabled={loading || !newTip.text.trim()}
                className="w-full bg-[#FFD60A] text-[#8B4513] py-4 rounded-2xl font-bold shadow-lg shadow-[#FFD60A]/20 hover:bg-[#FFD60A]/90 transition-all active:scale-95 disabled:opacity-50"
              >
                {loading ? <Loader2 className="animate-spin mx-auto" /> : 'Gửi lời khuyên'}
              </button>
            </form>
          </div>
        </div>
      ...
      {showRoadModal && (
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[24px] w-full max-w-md flex flex-col shadow-2xl">
            <div className="flex justify-between items-center p-4 border-b border-gray-100">
              <h2 className="text-[17px] font-semibold text-[#A0522D] flex items-center gap-2">
                <Accessibility size={20} /> Báo Cáo Đường Gồ Ghề
              </h2>
              <button onClick={() => setShowRoadModal(false)} className="p-1.5 text-[#8E8E93] hover:text-black bg-gray-100 rounded-full">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={async (e) => {
              e.preventDefault();
              if (!user || roadPoints.length < 2) return;
              setLoading(true);
              try {
                await addDoc(collection(db, 'rough_roads'), {
                  name: newRoad.name,
                  description: newRoad.description,
                  severity: newRoad.severity,
                ...
                setRoadPoints([]);
                setNewRoad({ name: '', description: '', severity: 'medium' });
              } catch (error) {
                handleFirestoreError(error, OperationType.CREATE, 'rough_roads');
              } finally {
                setLoading(false);
              }
            }} className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-semibold mb-1">Mức độ gồ ghề / Khó đi</label>
                <select 
                  className="w-full rounded-xl bg-gray-100 p-3 outline-none mb-4"
                  value={newRoad.severity}
                  onChange={e => setNewRoad({...newRoad, severity: e.target.value as any})}
                >
                  <option value="low">Thấp (Hơi khó đi, cần chú ý)</option>
                  <option value="medium">Trung bình (Gồ ghề, xe lăn khó di chuyển)</option>
                  <option value="high">Cao (Rất xấu, xe lăn không thể qua)</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">Tên đoạn đường</label>
                <input 
                  required
                  type="text" 
                  placeholder="VD: Đoạn đường đang thi công..."
                  className="w-full rounded-xl bg-gray-100 p-3 outline-none focus:ring-2 focus:ring-[#A0522D]/20"
                  value={newRoad.name}
              ...
              <div>
                ...
                <textarea 
                  placeholder="Mô tả tình trạng đường (nhiều ổ gà, không có vỉa hè...)"
                  className="w-full rounded-xl bg-gray-100 p-3 outline-none focus:ring-2 focus:ring-[#A0522D]/20 min-h-[100px]"
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-[#A0522D] text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2"
              >
        ...
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[24px] w-full max-w-md flex flex-col shadow-2xl">
            ...
            <form onSubmit={handleSaveDanger} className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-semibold mb-1">Loại cảnh báo</label>
                <select 
                  className="w-full rounded-xl bg-gray-100 p-3 outline-none mb-4"
                  value={newDanger.type}
                  onChange={e => setNewDanger({...newDanger, type: e.target.value as any})}
                >
                  <option value="danger">Khu vực nguy hiểm (An ninh, tội phạm...)</option>
                  <option value="rough_road">Đường gồ ghề / Khó đi (Cho xe lăn)</option>
                </select>
              </div>
              <div>
      ...
      <ConfirmModal
        ...
        title="Xác nhận xóa"
        message={
          deleteTarget?.type === 'danger_zone' ? 'Bạn có chắc chắn muốn xóa khu vực nguy hiểm này không? Hành động này không thể hoàn tác.' :
          deleteTarget?.type === 'place' ? 'Bạn có chắc chắn muốn xóa địa điểm an toàn này không? Hành động này không thể hoàn tác.' :
          deleteTarget?.type === 'safety_tip' ? 'Bạn có chắc chắn muốn xóa mẹo an toàn này không? Hành động này không thể hoàn tác.' :
